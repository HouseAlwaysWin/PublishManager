...
## 系統匣(關閉視窗不會結束程式）
按 **X 只會把視窗收到系統匣**,程式繼續在背景執行 —— 因為發版後 app 仍在輪詢 GitHub Actions,直接結束會中斷監看。

- **點系統匣圖示**或**再次啟動程式** → 視窗回來
- 真正要離開:系統匣圖示**右鍵 → 結束**

同一個使用者只會有**一份**程式在跑。第二次啟動不會開新的,而是把既有那份叫回前景 —— 否則兩份程式會同時輪詢、並且互相覆寫 `projects.json`。

> 首次使用時,系統匣圖示可能被 Windows 收在「顯示隱藏的圖示」(小箭頭)裡,可把它拖出來釘選。
